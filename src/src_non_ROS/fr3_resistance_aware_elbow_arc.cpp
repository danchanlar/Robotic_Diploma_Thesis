...
const char* modeName(RehabMode mode) {
  switch (mode) {
    ...
    case RehabMode::kLagAssistHold: return "LAG_ASSIST_HOLD";
...
struct Config {
  ...
  // Human elbow centre relative to the current handle position, expressed
  // in the FR3 base frame. Measure this vector for the actual setup.
  // The defaults assume: handle centre 0.20 m above the table, elbow-joint
  // centre approximately 0.04 m above the table, and elbow-to-handle radius
  // approximately 0.30 m. Therefore dz = 0.04 - 0.20 = -0.16 m and
  // dx = -sqrt(0.30^2 - 0.16^2) = approximately -0.253772 m.
  Eigen::Vector3d elbow_center_offset{-0.253772, 0.0, -0.160};
  Eigen::Vector3d rotation_axis{0.0, 1.0, 0.0};

  // The current pose is the upper start point. A positive 20.7 deg rotation
  // with the default geometry produces approximately [+0.040, 0, -0.100] m,
  // so the first half-cycle moves the handle from 0.20 m to 0.10 m.
  double amplitude_deg{20.7};
  double half_cycle_s{8.0};
  int repetitions{5};
  double settle_s{2.0};
  ...
  double lag_hold_tangent_stiffness_npm{10.0};
  double rotational_stiffness_nmprad{1.0};

  // Direction-dependent vertical support. Direction is inferred from the
  // actual start and end points of each half-cycle, rather than from odd/even
  // half-cycle indices. This is essential because the first half-cycle is now upward.
  double downward_vertical_support_force_n{0.5};
  double upward_vertical_support_force_n{2.5};
  ...
  double upward_assist_force_max_n{4.0};
  ...
  // Downward half-cycle thresholds.
  double force_free_n{0.8};
  double force_pause_n{3.0};
  double force_resume_n{1.2};

  // Upward half-cycle thresholds. The upward motion naturally carries more
  // load because the robot lifts the forearm. A configurable load offset is
  // removed before resistance classification, and higher pause thresholds are
  // used so normal arm weight is not interpreted as intentional resistance.
  double upward_opposing_force_offset_n{1.5};
  double upward_force_free_n{1.5};
  double upward_force_pause_n{5.0};
  double upward_force_resume_n{2.5};

  double force_filter_tau_s{0.10};
  double pause_persistence_s{0.15};
  double resume_persistence_s{0.25};
  double resistance_hold_timeout_s{4.0};
  double speed_scale_rate_per_s{2.0};

  // Tangential-lag handling. The path slows from warning to hold, freezes at
  // hold, and keeps bounded assistance active until the participant catches up.
  double tangential_warning_m{0.030};
  double tangential_hold_m{0.050};
  double tangential_resume_m{0.020};
  double tangential_hold_persistence_s{0.12};
  double tangential_resume_persistence_s{0.25};
  double lag_assist_hold_timeout_s{8.0};

  // Table geometry check. These are declared setup values, not sensor-derived.
  // The exercise starts at 0.20 m, descends to approximately 0.10 m, and returns.
  double initial_handle_height_m{0.200};
  double minimum_handle_height_m{0.080};
  double maximum_handle_height_m{0.220};
...
std::string stopReasonText(int reason) {
  switch (reason) {
    ...
    case 9: return "tangential lag did not recover during lag-assist hold";
...
std::filesystem::path createLogDirectory(const Config& cfg) {
  ...
  const std::filesystem::path directory =
      root / ("elbow_resistance_aware_" + timestampString());
...
void writeConfigCsv(const std::filesystem::path& path, const Config& cfg) {
  ...
  file << "elbow_dx_m," << cfg.elbow_center_offset.x() << '\n';
  file << "elbow_dy_m," << cfg.elbow_center_offset.y() << '\n';
  file << "elbow_dz_m," << cfg.elbow_center_offset.z() << '\n';
  ...
  file << "upward_assist_force_max_n," << cfg.upward_assist_force_max_n << '\n';
  file << "downward_vertical_support_force_n,"
       << cfg.downward_vertical_support_force_n << '\n';
  file << "upward_vertical_support_force_n,"
       << cfg.upward_vertical_support_force_n << '\n';
  file << "force_free_n," << cfg.force_free_n << '\n';
  file << "force_resume_n," << cfg.force_resume_n << '\n';
  file << "force_pause_n," << cfg.force_pause_n << '\n';
  file << "upward_opposing_force_offset_n,"
       << cfg.upward_opposing_force_offset_n << '\n';
  file << "upward_force_free_n," << cfg.upward_force_free_n << '\n';
  file << "upward_force_resume_n," << cfg.upward_force_resume_n << '\n';
  file << "upward_force_pause_n," << cfg.upward_force_pause_n << '\n';
  file << "tangential_warning_m," << cfg.tangential_warning_m << '\n';
  file << "tangential_hold_m," << cfg.tangential_hold_m << '\n';
  file << "tangential_resume_m," << cfg.tangential_resume_m << '\n';
  file << "initial_handle_height_m," << cfg.initial_handle_height_m << '\n';
  file << "minimum_handle_height_m," << cfg.minimum_handle_height_m << '\n';
  file << "maximum_handle_height_m," << cfg.maximum_handle_height_m << '\n';
...
void writeSummaryCsv(const std::filesystem::path& path,
  ...
  file << "lag_assist_hold_entries," << summary.lag_hold_entries << '\n';
...
void printUsage(const char* executable, const Config& cfg) {
  std::cout
      ...
      << "The current end-effector pose is the upper start position, approximately 0.20 m above the table. The 2.5 cm\n"
      << "handle must already be centred between the gripper fingers. The program\n"
      << "moves from approximately 0.20 m to 0.10 m and returns for the configured repetitions.\n\n"
      ...
      << "  --elbow-dx M                       default " << cfg.elbow_center_offset.x() << "\n"
      << "  --elbow-dy M                       default " << cfg.elbow_center_offset.y() << "\n"
      << "  --elbow-dz M                       default " << cfg.elbow_center_offset.z() << "\n"
      ...
      << "  --lag-hold-tangent-stiffness NPM      default " << cfg.lag_hold_tangent_stiffness_npm << "\n"
      ...
      << "  --upward-assist-force-max N           default " << cfg.upward_assist_force_max_n << "\n"
      << "  --downward-support-force N            default " << cfg.downward_vertical_support_force_n << "\n"
      << "  --upward-support-force N              default " << cfg.upward_vertical_support_force_n << "\n\n"
      << "Resistance response:\n"
      ...
      << "  --upward-force-offset N               default " << cfg.upward_opposing_force_offset_n << "\n"
      << "  --upward-force-free N                 default " << cfg.upward_force_free_n << "\n"
      << "  --upward-force-pause N                default " << cfg.upward_force_pause_n << "\n"
      << "  --upward-force-resume N               default " << cfg.upward_force_resume_n << "\n"
      ...
      << "  --tangential-warning M                default " << cfg.tangential_warning_m << "\n"
      << "  --tangential-hold M                   default " << cfg.tangential_hold_m << "\n"
      << "  --tangential-resume M                 default " << cfg.tangential_resume_m << "\n"
      << "  --tangential-hold-persistence S       default " << cfg.tangential_hold_persistence_s << "\n"
      << "  --tangential-resume-persistence S     default " << cfg.tangential_resume_persistence_s << "\n"
      << "  --lag-hold-timeout S                  default " << cfg.lag_assist_hold_timeout_s << "\n"
      << "  --initial-handle-height M             default " << cfg.initial_handle_height_m << "\n"
      << "  --minimum-handle-height M             default " << cfg.minimum_handle_height_m << "\n"
      << "  --maximum-handle-height M             default " << cfg.maximum_handle_height_m << "\n\n"
      << "Hard software stops:\n"
...
Config parseArguments(int argc, char** argv, Config cfg) {
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--elbow-dx") {
      cfg.elbow_center_offset.x() = parseDouble(next(arg), arg);
    } else if (arg == "--elbow-dy") {
      cfg.elbow_center_offset.y() = parseDouble(next(arg), arg);
    } else if (arg == "--elbow-dz") {
      cfg.elbow_center_offset.z() = parseDouble(next(arg), arg);
    } else if (arg == "--axis-x") {
    ...
    } else if (arg == "--lag-hold-tangent-stiffness") {
      cfg.lag_hold_tangent_stiffness_npm = parseDouble(next(arg), arg);
    ...
    } else if (arg == "--upward-assist-force-max") {
      cfg.upward_assist_force_max_n = parseDouble(next(arg), arg);
    } else if (arg == "--downward-support-force") {
      cfg.downward_vertical_support_force_n = parseDouble(next(arg), arg);
    } else if (arg == "--upward-support-force") {
      cfg.upward_vertical_support_force_n = parseDouble(next(arg), arg);
    } else if (arg == "--vertical-support-force") {
      // Backward-compatible option: apply the same support in both directions.
      const double value = parseDouble(next(arg), arg);
      cfg.downward_vertical_support_force_n = value;
      cfg.upward_vertical_support_force_n = value;
    ...
    } else if (arg == "--upward-force-offset") {
      cfg.upward_opposing_force_offset_n = parseDouble(next(arg), arg);
    } else if (arg == "--upward-force-free") {
      cfg.upward_force_free_n = parseDouble(next(arg), arg);
    } else if (arg == "--upward-force-pause") {
      cfg.upward_force_pause_n = parseDouble(next(arg), arg);
    } else if (arg == "--upward-force-resume") {
      cfg.upward_force_resume_n = parseDouble(next(arg), arg);
    ...
    } else if (arg == "--tangential-warning") {
      cfg.tangential_warning_m = parseDouble(next(arg), arg);
    } else if (arg == "--tangential-hold") {
      cfg.tangential_hold_m = parseDouble(next(arg), arg);
    } else if (arg == "--tangential-resume") {
      cfg.tangential_resume_m = parseDouble(next(arg), arg);
    } else if (arg == "--tangential-hold-persistence") {
      cfg.tangential_hold_persistence_s = parseDouble(next(arg), arg);
    } else if (arg == "--tangential-resume-persistence") {
      cfg.tangential_resume_persistence_s = parseDouble(next(arg), arg);
    } else if (arg == "--lag-hold-timeout") {
      cfg.lag_assist_hold_timeout_s = parseDouble(next(arg), arg);
    } else if (arg == "--initial-handle-height") {
      cfg.initial_handle_height_m = parseDouble(next(arg), arg);
    } else if (arg == "--minimum-handle-height") {
      cfg.minimum_handle_height_m = parseDouble(next(arg), arg);
    } else if (arg == "--maximum-handle-height") {
      cfg.maximum_handle_height_m = parseDouble(next(arg), arg);
  ...
  const double radius = cfg.elbow_center_offset.norm();
  if (radius < 0.18 || radius > 0.45) {
    throw std::runtime_error("Elbow-to-handle radius must be in [0.18, 0.45] m.");
  }
  const Eigen::Vector3d axis = cfg.rotation_axis.normalized();
  const Eigen::Vector3d initial_radius = -cfg.elbow_center_offset;
  if (std::abs(axis.dot(initial_radius.normalized())) > 0.10) {
    throw std::runtime_error(
        "For this planar elbow arc, the rotation axis must be approximately "
        "perpendicular to the elbow-to-handle radius.");
  }
  if (std::abs(cfg.amplitude_deg) > 70.0) {
    throw std::runtime_error("This prototype refuses elbow amplitudes above 70 degrees.");
  }
  ...
  if (!stiffness_valid(cfg.stiffness_tangent_npm) ||
      ...
      !stiffness_valid(cfg.lag_hold_tangent_stiffness_npm)) {
  ...
  if (cfg.downward_vertical_support_force_n < 0.0 ||
      cfg.downward_vertical_support_force_n > 8.0 ||
      cfg.upward_vertical_support_force_n < 0.0 ||
      cfg.upward_vertical_support_force_n > 8.0) {
    throw std::runtime_error(
        "Downward and upward support forces must be in [0, 8] N.");
  }
  if (cfg.assist_stiffness_npm < 0.0 || cfg.assist_stiffness_npm > 200.0 ||
      cfg.assist_force_max_n < 0.0 || cfg.assist_force_max_n > 8.0 ||
      cfg.upward_assist_force_max_n < 0.0 ||
      cfg.upward_assist_force_max_n > 8.0 ||
  ...
  if (cfg.upward_opposing_force_offset_n < 0.0 ||
      cfg.upward_opposing_force_offset_n > 6.0 ||
      cfg.upward_force_free_n < 0.0 ||
      cfg.upward_force_pause_n <= cfg.upward_force_free_n ||
      cfg.upward_force_resume_n < cfg.upward_force_free_n ||
      cfg.upward_force_resume_n >= cfg.upward_force_pause_n ||
      cfg.upward_force_pause_n >= cfg.force_delta_hard_stop_n) {
    throw std::runtime_error(
        "Invalid upward resistance parameters. Require non-negative offset and "
        "0 <= upward-force-free <= upward-force-resume < upward-force-pause "
        "< force-delta-hard-stop.");
  }
  ...
  if (!(0.0 < cfg.tangential_resume_m &&
        cfg.tangential_resume_m < cfg.tangential_warning_m &&
        cfg.tangential_warning_m < cfg.tangential_hold_m &&
        cfg.tangential_hold_m < cfg.tangential_error_hard_stop_m)) {
    throw std::runtime_error(
        "Require 0 < tangential-resume < tangential-warning < tangential-hold < tangential hard stop.");
  }
  if (cfg.tangential_hold_persistence_s < 0.02 ||
      cfg.tangential_hold_persistence_s > 1.0 ||
      cfg.tangential_resume_persistence_s < 0.02 ||
      cfg.tangential_resume_persistence_s > 2.0 ||
      cfg.lag_assist_hold_timeout_s < 0.5 ||
      cfg.lag_assist_hold_timeout_s > 30.0) {
    throw std::runtime_error("Invalid tangential-lag timing parameters.");
  }
  if (cfg.minimum_handle_height_m <= 0.0 ||
      cfg.initial_handle_height_m < cfg.minimum_handle_height_m ||
      cfg.maximum_handle_height_m <= cfg.initial_handle_height_m) {
    throw std::runtime_error(
        "Require 0 < minimum-handle-height <= initial-handle-height < maximum-handle-height.");
  }
...
ArcSample sampleElbowArc(const Config& cfg,
                            const Eigen::Vector3d& initial_position,
                            int half_cycle_index,
                            double phase,
                            double phase_dot) {
  const Eigen::Vector3d axis = cfg.rotation_axis.normalized();
  const Eigen::Vector3d center = initial_position + cfg.elbow_center_offset;
  ...
  const bool outbound_segment = (half_cycle_index % 2) == 0;
  const double start_phi = outbound_segment ? 0.0 : amplitude;
  const double end_phi = outbound_segment ? amplitude : 0.0;
...
bool isUpwardHalfCycle(const Config& cfg,
                       const Eigen::Vector3d& initial_position,
                       int half_cycle_index) {
  const ArcSample start =
      sampleElbowArc(cfg, initial_position, half_cycle_index, 0.0, 0.0);
  const ArcSample end =
      sampleElbowArc(cfg, initial_position, half_cycle_index, 1.0, 0.0);
  return end.position.z() > start.position.z();
}

RunSummary runElbowExercise(franka::Robot& robot, const Config& cfg) {
  ...
  const ArcSample first_endpoint_sample =
      sampleElbowArc(cfg, initial_position, 0, 1.0, 0.0);
  const Eigen::Vector3d first_displacement =
      first_endpoint_sample.position - initial_position;
  const double predicted_endpoint_height =
      cfg.initial_handle_height_m + first_displacement.z();

  std::cout << std::fixed << std::setprecision(3)
            << "Predicted first-half-cycle displacement [m]: "
            << first_displacement.transpose() << "\n"
            << "Declared initial handle height above table: "
            << cfg.initial_handle_height_m << " m\n"
            << "Predicted first endpoint height above table: "
            << predicted_endpoint_height << " m\n"
            << "Downward/upward support forces: "
            << cfg.downward_vertical_support_force_n << " / "
            << cfg.upward_vertical_support_force_n << " N\n"
            << "Upward opposing-force offset: "
            << cfg.upward_opposing_force_offset_n << " N\n";

  if (first_displacement.z() >= -0.005) {
    throw std::runtime_error(
        "The first half-cycle is not clearly downward. Reverse the sign of "
        "--amplitude-deg or correct the elbow-centre geometry.");
  }
  if (cfg.initial_handle_height_m < cfg.minimum_handle_height_m ||
      cfg.initial_handle_height_m > cfg.maximum_handle_height_m) {
    throw std::runtime_error(
        "The declared initial handle height lies outside the configured table-height limits.");
  }
  if (predicted_endpoint_height < cfg.minimum_handle_height_m) {
    throw std::runtime_error(
        "The predicted lower endpoint is below the configured minimum table clearance.");
  }
  if (predicted_endpoint_height > cfg.maximum_handle_height_m) {
    throw std::runtime_error(
        "The predicted first endpoint exceeds the configured maximum handle height.");
  }
  ...
  double tangential_lag_exceedance_s = 0.0;
  double tangential_lag_resume_s = 0.0;
  double lag_hold_elapsed_s = 0.0;
  ...
  robot.control([&](const franka::RobotState& state,
    ...
    ArcSample sample = sampleElbowArc(
        cfg, initial_position, sample_half_cycle, phase, phase_dot_for_projection);

    // Infer direction from the actual endpoint heights. Do not assume that
    // odd or even half-cycles are upward, because the configured first motion
    // may be upward or downward.
    const bool upward_half_cycle =
        isUpwardHalfCycle(cfg, initial_position, sample_half_cycle);
    const double active_force_free_n =
        upward_half_cycle ? cfg.upward_force_free_n : cfg.force_free_n;
    const double active_force_resume_n =
        upward_half_cycle ? cfg.upward_force_resume_n : cfg.force_resume_n;
    const double active_force_pause_n =
        upward_half_cycle ? cfg.upward_force_pause_n : cfg.force_pause_n;

    const double projected_force_n =
        cfg.force_projection_sign * sample.tangent.dot(delta_force_base);
    const double raw_opposing_force_n = std::max(0.0, projected_force_n);

    // During upward motion, part of the projected force is the expected load
    // from lifting the forearm rather than voluntary resistance. Remove only a
    // bounded, configured offset before resistance classification. Hard force
    // safety checks still use the unmodified measured force.
    const double opposing_force_n = std::max(
        0.0,
        raw_opposing_force_n -
            (upward_half_cycle ? cfg.upward_opposing_force_offset_n : 0.0));
    ...
    const Eigen::Vector3d preview_position_error = sample.position - position;
    const double preview_tangential_error_m =
        sample.tangent.dot(preview_position_error);
    const double positive_tangential_lag_m =
        std::max(0.0, preview_tangential_error_m);

    bool resistance_hold_timeout = false;
    bool lag_hold_timeout = false;

    if (!calibrating && mode != RehabMode::kFinalHold) {
      if (mode == RehabMode::kCompliantHold) {
        hold_elapsed_s += dt;
        if (filtered_opposing_force_n <= active_force_resume_n) {
        ...
        if (resume_below_threshold_s >= cfg.resume_persistence_s) {
          mode = positive_tangential_lag_m >= cfg.tangential_hold_m
                     ? RehabMode::kLagAssistHold
                     : (filtered_opposing_force_n <= active_force_free_n
                            ? RehabMode::kMove
                            : RehabMode::kSlowAssist);
          hold_elapsed_s = 0.0;
          resume_below_threshold_s = 0.0;
          pause_exceedance_s = 0.0;
          lag_hold_elapsed_s = 0.0;
          tangential_lag_resume_s = 0.0;
        } else if (hold_elapsed_s >= cfg.resistance_hold_timeout_s) {
          resistance_hold_timeout = true;
        }
      } else if (mode == RehabMode::kLagAssistHold) {
        lag_hold_elapsed_s += dt;

        // High opposing force has priority: do not push through resistance.
        if (filtered_opposing_force_n >= active_force_pause_n) {
        ...
        if (pause_exceedance_s >= cfg.pause_persistence_s) {
          ...
          if (positive_tangential_lag_m <= cfg.tangential_resume_m) {
            tangential_lag_resume_s += dt;
          } else {
            tangential_lag_resume_s = 0.0;
          }

          if (tangential_lag_resume_s >= cfg.tangential_resume_persistence_s) {
            mode = filtered_opposing_force_n <= active_force_free_n
                       ? RehabMode::kMove
                       : RehabMode::kSlowAssist;
            lag_hold_elapsed_s = 0.0;
            tangential_lag_resume_s = 0.0;
            tangential_lag_exceedance_s = 0.0;
          } else if (lag_hold_elapsed_s >= cfg.lag_assist_hold_timeout_s) {
            lag_hold_timeout = true;
          }
        }
      } else {
        if (filtered_opposing_force_n >= active_force_pause_n) {
        ...
        if (positive_tangential_lag_m >= cfg.tangential_hold_m) {
          tangential_lag_exceedance_s += dt;
        } else {
          tangential_lag_exceedance_s = 0.0;
        }
        ...
        if (pause_exceedance_s >= cfg.pause_persistence_s) {
          ...
          resume_below_threshold_s = 0.0;
          ++summary.hold_entries;
        } else if (tangential_lag_exceedance_s >=
                   cfg.tangential_hold_persistence_s) {
          mode = RehabMode::kLagAssistHold;
          speed_scale = 0.0;
          lag_hold_elapsed_s = 0.0;
          tangential_lag_resume_s = 0.0;
          ++summary.lag_hold_entries;
        } else if (filtered_opposing_force_n > active_force_free_n ||
                   positive_tangential_lag_m > cfg.tangential_warning_m) {
    ...
    const double force_speed_scale =
        filtered_opposing_force_n <= active_force_free_n
            ? 1.0
            : std::clamp(
                  (active_force_pause_n - filtered_opposing_force_n) /
                      (active_force_pause_n - active_force_free_n),
                  0.0,
                  1.0);

    double lag_speed_scale = 1.0;
    if (positive_tangential_lag_m > cfg.tangential_warning_m) {
      lag_speed_scale = std::clamp(
          (cfg.tangential_hold_m - positive_tangential_lag_m) /
              (cfg.tangential_hold_m - cfg.tangential_warning_m),
          0.0,
          1.0);
    }
    ...
    if (mode == RehabMode::kMove || mode == RehabMode::kSlowAssist) {
      speed_scale_target = std::min(force_speed_scale, lag_speed_scale);
    }
    ...
    if (!calibrating &&
      ...
      if (phase >= 1.0) {
        ...
        // Clear directional resistance state at each reversal. Otherwise the
        // force accumulated during one direction can unnecessarily slow
        // or pause the following reversal.
        filtered_opposing_force_n = 0.0;
        pause_exceedance_s = 0.0;
        resume_below_threshold_s = 0.0;
        hold_elapsed_s = 0.0;
    ...
    double active_tangent_stiffness = cfg.stiffness_tangent_npm;
    if (mode == RehabMode::kCompliantHold) {
      active_tangent_stiffness = cfg.hold_tangent_stiffness_npm;
    } else if (mode == RehabMode::kLagAssistHold) {
      active_tangent_stiffness = cfg.lag_hold_tangent_stiffness_npm;
    }
    ...
    if (mode == RehabMode::kMove || mode == RehabMode::kSlowAssist ||
      ...
      const bool command_upward_half_cycle =
          isUpwardHalfCycle(cfg, initial_position, command_half_cycle);
      const double active_assist_force_max_n =
          command_upward_half_cycle
              ? cfg.upward_assist_force_max_n
              : cfg.assist_force_max_n;
      assist_force_n = std::clamp(
          cfg.assist_stiffness_npm * positive_lag_m,
          0.0,
          active_assist_force_max_n);
    ...
    if (g_stop_requested != 0) {
      ...
      stop = true;
    } else if (lag_hold_timeout) {
      summary.stop_reason = 9;
      stop = true;
  ...
  std::cout << std::fixed << std::setprecision(3)
            ...
            << "Lag-assist-hold entries: " << summary.lag_hold_entries << "\n"
  ...
  switch (summary.stop_reason) {
    ...
    case 8:
      ...
      break;
    case 9:
      std::cout << "Stop reason: tangential lag did not recover during lag-assist hold.\n";
      break;
    default:
...
int main(int argc, char** argv) {
  ...
  std::cout
      << "\n=== FR3 resistance-aware elbow flexion-extension exercise ===\n"
      << "The current pose is the lower start position, approximately 0.10 m above the table. No automatic approach is used.\n"
      ...
      << "Elbow-centre offset [m]: "
      << cfg.elbow_center_offset.transpose() << "\n"
      << "Rotation axis: " << cfg.rotation_axis.transpose() << "\n"
      << "Amplitude: " << cfg.amplitude_deg << " deg\n"
      << "Nominal half-cycle: " << cfg.half_cycle_s << " s\n"
      << "Repetitions: " << cfg.repetitions << "\n"
      << "Downward resistance thresholds [free/resume/pause]: "
      << cfg.force_free_n << " / " << cfg.force_resume_n << " / "
      << cfg.force_pause_n << " N\n"
      << "Upward resistance thresholds [free/resume/pause]: "
      << cfg.upward_force_free_n << " / " << cfg.upward_force_resume_n
      << " / " << cfg.upward_force_pause_n << " N\n"
      << "Upward opposing-force offset: "
      << cfg.upward_opposing_force_offset_n << " N\n"
      << "Support force [down/up]: "
      << cfg.downward_vertical_support_force_n << " / "
      << cfg.upward_vertical_support_force_n << " N\n"
      << "Assist maximum [down/up]: "
      << cfg.assist_force_max_n << " / "
      << cfg.upward_assist_force_max_n << " N\n"
      << "Tangential lag thresholds [resume/warning/hold/hard]: "
      << cfg.tangential_resume_m << " / " << cfg.tangential_warning_m
      << " / " << cfg.tangential_hold_m << " / "
      << cfg.tangential_error_hard_stop_m << " m\n"
      << "Declared handle height [initial/minimum/maximum]: "
      << cfg.initial_handle_height_m << " / " << cfg.minimum_handle_height_m
      << " / " << cfg.maximum_handle_height_m << " m\n"
  ...
  try {
    ...
    if (!promptExact(
            "The participant may now hold the handle at the lower 0.10 m start point. The elbow centre "
            "must be physically positioned on the tabletop, preferably at least 0.05 m inside the edge, but must not be strapped or mechanically locked. "
            "The shoulder and upper arm must remain supported and stationary. Keep the participant relaxed "
            "during the initial force-bias period. Type READY: ",
            "READY")) {
    ...
    std::cout
        << "Starting upward-first resistance-aware elbow flexion-extension arc. Mild resistance or lag slows the path; "
        << "high resistance causes a compliant hold, while excessive lag causes a bounded lag-assist hold. Ctrl+C or the "
        << "physical user stop can interrupt it.\n";

    const RunSummary summary = runElbowExercise(robot, cfg);
